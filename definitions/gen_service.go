...
func (gs *genService) buildImplemented(path string) {
	...
	for _, v := range fi {
		...
		// Skip files excluded by the current platform's build constraints
		// (e.g. _windows.go on Linux) to avoid parsing errors from
		// platform-specific code patterns like func-type parameters.
		matched, err := build.Default.MatchFile(base, v.Name())
		if err != nil || !matched {
			continue
		}
		...
		// Skip external test packages (package name ends with "_test") since
		// they may contain func-type parameters that templateutils cannot handle,
		// and they never define *Factory/*Service/*Storage methods.
		if strings.HasSuffix(v.Name(), "_test.go") {
			fset := token.NewFileSet()
			pf, parseErr := parser.ParseFile(fset, v.Name(), content, parser.PackageClauseOnly)
			if parseErr != nil || strings.HasSuffix(pf.Name.Name, "_test") {
				continue
			}
		}
...
func (gs *genService) generateHeader() {
	...
	imp := f.NewImport().
		...
		AddPath("errors").
		AddLine().
		AddPath("github.com/rgglez/go-storage/v5/services").
		AddPath("github.com/rgglez/go-storage/v5/types")

	for _, path := range gs.data.Imports {
		imp.AddPath(path)
	}
