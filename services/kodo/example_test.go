...
//
// Example code for the kodo (Qiniu Cloud Object Storage) service.
//
...
// ExampleNewStorager demonstrates how to initialize a Qiniu Kodo backend.
//
//	export KODO_CREDENTIAL=hmac:ACCESS_KEY:SECRET_KEY
//	export KODO_ENDPOINT=https://up.qiniup.com
//	export KODO_BUCKET=my-bucket
func ExampleNewStorager() {
	ps := []types.Pair{
		pairs.WithCredential(os.Getenv("KODO_CREDENTIAL")),
		pairs.WithEndpoint(os.Getenv("KODO_ENDPOINT")),
		pairs.WithName(os.Getenv("KODO_BUCKET")),
	}
	if wd := os.Getenv("KODO_WORK_DIR"); wd != "" {
...
func ExampleStorage_Write() {
	_, store, err := kodo.New(
		pairs.WithCredential("hmac:ACCESS_KEY:SECRET_KEY"),
		pairs.WithEndpoint("https://up.qiniup.com"),
	...
	const content = "Hello from go-storage kodo example!"
...
// ExampleServicer_Create demonstrates creating a new Kodo bucket via the Servicer.
