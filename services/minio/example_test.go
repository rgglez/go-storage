...
//
// Example code for the minio (MinIO / S3-compatible) service.
//
...
// ExampleNewStorager demonstrates how to initialize a MinIO backend.
//
//	export MINIO_CREDENTIAL=hmac:ACCESS_KEY:SECRET_KEY
//	export MINIO_ENDPOINT=https://play.min.io
//	export MINIO_BUCKET=my-bucket
func ExampleNewStorager() {
	ps := []types.Pair{
		pairs.WithCredential(os.Getenv("MINIO_CREDENTIAL")),
		pairs.WithEndpoint(os.Getenv("MINIO_ENDPOINT")),
		pairs.WithName(os.Getenv("MINIO_BUCKET")),
	}
	if wd := os.Getenv("MINIO_WORK_DIR"); wd != "" {
...
func ExampleStorage_Write() {
	_, store, err := minio.New(
		pairs.WithCredential("hmac:ACCESS_KEY:SECRET_KEY"),
		pairs.WithEndpoint("https://play.min.io"),
	...
	const content = "Hello from go-storage minio example!"
...
// ExampleServicer_Create demonstrates creating a new MinIO bucket via the Servicer.
