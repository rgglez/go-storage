...
//
// Example code for the uss (UPYUN Storage Service) service.
//
...
// ExampleNewStorager demonstrates how to initialize a UPYUN Storage backend.
//
//	export USS_CREDENTIAL=hmac:OPERATOR:PASSWORD
//	export USS_BUCKET=my-bucket
func ExampleNewStorager() {
	ps := []types.Pair{
		pairs.WithCredential(os.Getenv("USS_CREDENTIAL")),
		pairs.WithName(os.Getenv("USS_BUCKET")),
	}
...
// The USS credential format is "hmac:OPERATOR:PASSWORD".
func ExampleStorage_Write() {
	store, err := uss.NewStorager(
		pairs.WithCredential("hmac:OPERATOR:PASSWORD"),
