...
//
// Example code for the tar (TAR archive storage) service.
//
...
// ExampleNewStorager demonstrates how to initialize a TAR archive backend.
//
// The endpoint is the path to the TAR file using the "file:" scheme.
//
//	export TAR_FILE=file:/path/to/archive.tar
func ExampleNewStorager() {
	ps := []types.Pair{
		// Endpoint: path to the TAR archive using the file: URI scheme.
		pairs.WithEndpoint(os.Getenv("TAR_FILE")),
	}
...
// against a TAR archive.
func ExampleStorage_Write() {
	store, err := tar.NewStorager(
		pairs.WithEndpoint("file:/tmp/myarchive.tar"),
	)
	if err != nil {
		return
	}
