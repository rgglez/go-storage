...
//
// Example code for the azfile (Azure File Storage) service.
//
...
// ExampleNewStorager demonstrates how to initialize an Azure File Storage backend.
//
//	export AZFILE_CREDENTIAL=hmac:ACCOUNT_NAME:ACCOUNT_KEY
//	export AZFILE_ENDPOINT=https://ACCOUNT_NAME.file.core.windows.net
//	export AZFILE_SHARE=my-share
func ExampleNewStorager() {
	ps := []types.Pair{
		pairs.WithCredential(os.Getenv("AZFILE_CREDENTIAL")),
		pairs.WithEndpoint(os.Getenv("AZFILE_ENDPOINT")),
		pairs.WithName(os.Getenv("AZFILE_SHARE")),
	}
...
func ExampleStorage_Write() {
	store, err := azfile.NewStorager(
		...
		pairs.WithEndpoint("https://ACCOUNT_NAME.file.core.windows.net"),
		pairs.WithName("my-share"),
	)
	if err != nil {
		return
	}
