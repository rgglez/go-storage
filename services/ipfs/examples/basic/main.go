...
//
// Basic example for the ipfs (InterPlanetary File System) service.
//
...
//
//	IPFS_ENDPOINT  – IPFS API address (e.g. http://127.0.0.1:5001)
//	IPFS_GATEWAY   – IPFS HTTP gateway (e.g. http://127.0.0.1:8080)
//
// Optional:
//
//	IPFS_WORK_DIR  – MFS (Mutable File System) path prefix
package main
...
func main() {
	endpoint := requireEnv("IPFS_ENDPOINT")
	gateway := requireEnv("IPFS_GATEWAY")

	ps := []types.Pair{
		pairs.WithEndpoint(endpoint),
		ipfs.WithGateway(gateway),
	}
