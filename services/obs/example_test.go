...
//
// Example code for the obs (Huawei Cloud OBS) service.
//
...
// ExampleNewStorager demonstrates how to initialize a Huawei Cloud OBS backend.
//
//	export OBS_CREDENTIAL=hmac:ACCESS_KEY:SECRET_KEY
//	export OBS_ENDPOINT=https://obs.cn-north-4.myhuaweicloud.com
//	export OBS_BUCKET=my-bucket
func ExampleNewStorager() {
	ps := []types.Pair{
		pairs.WithCredential(os.Getenv("OBS_CREDENTIAL")),
		pairs.WithEndpoint(os.Getenv("OBS_ENDPOINT")),
		pairs.WithName(os.Getenv("OBS_BUCKET")),
	}
	if wd := os.Getenv("OBS_WORK_DIR"); wd != "" {
		ps = append(ps, pairs.WithWorkDir(wd))
	}

	_, store, err := obs.New(ps...)
	if err != nil {
		fmt.Printf("ERROR: %v\n", err)
		return
	}
...
func ExampleStorage_Write() {
	_, store, err := obs.New(
		...
		pairs.WithEndpoint("https://obs.cn-north-4.myhuaweicloud.com"),
	...
	const content = "Hello from go-storage obs example!"
...
// ExampleServicer_Create demonstrates creating a new OBS bucket via the Servicer.
